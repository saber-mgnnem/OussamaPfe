...
const Profile = () => {
  ...
  if (errorMessage) {
    ...
    try {
      const config = {
        ...
        axios.post(`/api/AdminUpdate_user/${id}`,  {  fisrtname,lastname, email,role, phone,address,country,file},config).then(res => {
  ...
  const handleUpdatePassword = () => {
    axios.put(`/api/admin_updatePassword/${authUser.id}`, { newPassword })
      .then(response => {
        if (response.data.status === 200) {
