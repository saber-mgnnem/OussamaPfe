...
const Profile = () => {
  ...
  useEffect(() => {
    axios.get(`/api/Etudiant_get_data/${id}`).then(response => {
  ...
  if (errorMessage) {
    ...
    try {
      const config = {
        ...
        axios.post(`/api/Etudiant_Update_profile/${id}`,  {  fisrtname,lastname, email,role, phone,address,country,file},config).then(res => {
  ...
  const handleUpdatePassword = () => {
    axios.put(`/api/Etudiant_updatePassword/${authUser.id}`, { newPassword })
      .then(response => {
        if (response.data.status === 200) {
